    ...
    if (res.statusCode != 200) {
      ...
      for (var element in list) {
        if (element.contains("csrftoken")) {
          ...
          csrfToken.add(csrf[1]);
        } else if (element.contains("sessionid")) {
          var session = element.split(",");
          sessionId.add(session[1]);
        } else if (element.contains("SERVERID33516")) {
          server.add(element);
        }
      }

      var csrfToken_2 = csrfToken[0].substring(10);
      // print(csrfToken);
      // print(sessionId);
      // print(server);
      ...
      localStorage.setString('session_id', sessionId[0].toString());
      localStorage.setString("csrf_token", csrfToken[0].toString());
      ...
      String url2 = "http://valzyzt.cluster024.hosting.ovh.net/api/v1/student/";

      var res = await http.get(url2, headers: {
        'Content-Type': 'application/json',
        "Accept": "application/json",
        "Cookie": "${sessionId[0]}; ${csrfToken[0]}; ${server[0]}",
